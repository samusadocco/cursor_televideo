...
class ZDFHtmlTeletextViewer extends StatefulWidget {
  final TelevideoPage page;
  final Function(int pageNumber)? onPageNavigation;
  final VoidCallback? onTap;

  const ZDFHtmlTeletextViewer({
    super.key,
    required this.page,
  ...
  @override
  State<ZDFHtmlTeletextViewer> createState() => _ZDFHtmlTeletextViewerState();
}

class _ZDFHtmlTeletextViewerState extends State<ZDFHtmlTeletextViewer> {
  ...
  @override
  void didUpdateWidget(ZDFHtmlTeletextViewer oldWidget) {
  ...
  Future<void> _extractContent() async {
    try {
      ...
      print('[ZDFHtmlTeletextViewer] Using pre-fetched HTML content');
      ...
      if (widget.page.htmlContent != null) {
        ...
        print('[ZDFHtmlTeletextViewer] HTML content length: ${_rawHtmlContent?.length ?? 0}');
      ...
      print('[ZDFHtmlTeletextViewer] Error loading content: $e');
      setState(() {
        _isLoading = false;
      });
    }
  ...
  void _initializeOrUpdateWebView(double width, double height) {
    // Determina le dimensioni native in base al provider
    double nativeWidth;
    double nativeHeight;
    
    if (widget.page.providerId == 'nos_teletekst') {
      // NOS Teletekst (Olanda) - dimensioni native del div teletext
      nativeWidth = 480.0;
      nativeHeight = 580.0;
    } else {
      // ZDF Teletext (Germania) - dimensioni native standard
      nativeWidth = 492.0;
      nativeHeight = 489.0;
    }
    ...
    print('[ZDFHtmlTeletextViewer] Widget size: ${width}x$height (real available space)');
    print('[ZDFHtmlTeletextViewer] Native content: ${nativeWidth}x$nativeHeight');
    print('[ZDFHtmlTeletextViewer] Provider: ${widget.page.providerId}');
    print('[ZDFHtmlTeletextViewer] Calculated scales - X: $scaleX, Y: $scaleY');
    ...
    if (_controller == null) {
      ...
      ..addJavaScriptChannel(
        ...
        onMessageReceived: (JavaScriptMessage message) {
          ...
          print('[ZDFHtmlTeletextViewer] PageNavigation received: $pageNumber');
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          onPageFinished: (url) {
            print('[ZDFHtmlTeletextViewer] Page loaded');
            if (mounted) {
              setState(() {
                _isLoading = false;
              });
  ...
  String _buildHtmlString(double scaleX, double scaleY) {
    ...
    html = html.replaceAllMapped(
      ...
      (match) {
        ...
        print('[ZDFHtmlTeletextViewer] Converting CSS path: $relativePath -> $absolutePath');
    ...
    html = html.replaceAllMapped(
      ...
      (match) {
        ...
        final absolutePath = _makeAbsoluteUrl(relativePath);
        print('[ZDFHtmlTeletextViewer] Converting font path: $relativePath -> $absolutePath');
