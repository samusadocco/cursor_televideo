...
class IAPService {
  ...
  List<IAPProduct> get products => _toIapProducts(_storeProducts);
  ...
  Future<void> _loadProducts() async {
    try {
      ...
      print('[IAPService] Loaded ${_storeProducts.length} products');
  ...
  IAPProduct? getMonthlyProduct() => _findIapProduct(_monthlyProductId);

  /// Ottiene il prodotto trimestrale
  IAPProduct? getQuarterlyProduct() => _findIapProduct(_quarterlyProductId);

  /// Ottiene un prodotto specifico per ID
  IAPProduct? getProductById(String productId) => _findIapProduct(productId);

  /// Ottiene tutti i prodotti disponibili
  List<IAPProduct> getAllProducts() => products;

  /// [Deprecated] Usa getQuarterlyProduct() invece
  @Deprecated('Use getMonthlyProduct() or getQuarterlyProduct()')
  IAPProduct? getPremiumProduct() => getQuarterlyProduct();

  IAPProduct? _findIapProduct(String productId) {
    try {
      final product = _storeProducts.firstWhere((p) => p.id == productId);
      return _toIapProduct(product);
    } catch (e) {
      return null;
    }
  }

  ProductDetails? _findStoreProduct(String productId) {
    try {
      return _storeProducts.firstWhere((p) => p.id == productId);
    } catch (e) {
      return null;
    }
  }

  IAPProduct _toIapProduct(ProductDetails product) {
    return IAPProduct(
      id: product.id,
      title: product.title,
  ...
  List<IAPProduct> _toIapProducts(List<ProductDetails> products) {
    return products.map(_toIapProduct).toList();
  }
  ...
  Future<bool> purchasePremium({String? productId}) async {
    try {
      ...
      final product = _findStoreProduct(targetProductId);
      if (product == null) {
